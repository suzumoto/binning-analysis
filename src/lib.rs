...
where
    T: std::convert::From<u32> + std::fmt::Debug + std::convert::From<f64>,
{
    pub fn new() -> Self {
        Self {
            dataset: Vec::new(),
        ...
        match self.dataset.len() {
            ...
            num_sample @ 1...31 => {
                let mean = self
                    .dataset
                    .iter()
                    .fold(num::zero::<T>(), |sum, &value| sum + value)
                    / (num_sample as u32).into();
                let naive_variance = self
                    .dataset
                    .iter()
                    .fold(num::zero::<T>(), |variance, &value| {
                        variance + (value - mean) * (value - mean)
                    })
                    / (num_sample as f64 * (num_sample - 1) as f64).into();
            ...
            num_sample => {
                ...
                // calculate 32 bins
                {
                    ...
                    for bin_index in 0..number_of_bins {
                        bin_mean.push(
                            self.dataset
                                [bin_index * samples_in_bin..(bin_index + 1) * samples_in_bin]
                                .iter()
                                .fold(num::zero::<T>(), |sum, &value| sum + value)
                                / (samples_in_bin as f64).into(),
                        );
                    }
                    let variance_32 = bin_mean.iter().fold(num::zero::<T>(), |variance, &value| {
                        variance + (value - mean) * (value - mean)
                    })
                        / (number_of_bins as f64 * (number_of_bins - 1) as f64).into();
                ...
                // calculate 16 bins
                {
                    ...
                    let variance_16 = bin_mean.iter().fold(num::zero::<T>(), |variance, &value| {
                        variance + (value - mean) * (value - mean)
                    })
                        / (number_of_bins as f64 * (number_of_bins - 1) as f64).into();
                    error_16 = variance_16.sqrt();
                    correlation_time = variance_16 / naive_variance;
                    if error_32 > error_16 * 1.05.into() {
                        // should not occur
                        converged = false;
                    } else {
                        converged = true;
...
fn test_format() {
    ...
    assert_eq!(
        format!("{}", an),
        "12.1 +- 0.3, tau: 1.1 ......converged for 1024 entries"
    );
}
